...
def check_cpp_file(cpp_filepath):
    """
    Reads a C++ file from the given path, checks for bugs using Cppcheck,
    and returns a report of the findings.

    Args:
        cpp_filepath: The path to the C++ file.
    ...
    if not os.path.exists(cpp_filepath):
        print(f"Error: C++ file not found: {cpp_filepath}")
        return None
    ...
    try:
        ...
        command = [
            ...
            "-i",
            "C:/Users/GLAND COMPUTER/hust-codexity/sast-agent/SampleFile/Test.h",
            cpp_filepath
        ]
        ...
        if os.path.exists('cppcheck_report.xml'):
            ...
            print(f"Error: An error has occured. file cppcheck_report.xml does not exists")
        ...
        print("Error: Cppcheck not found. Please ensure it is installed and in your PATH.")
        return None
...
if __name__ == "__main__":
    # The path to the C++ file to check.
    cpp_file_path = "C:/Users/GLAND COMPUTER/hust-codexity/sast-agent/SampleFile/Test.cpp"
    header_file_path = "C:/Users/GLAND COMPUTER/hust-codexity/sast-agent/SampleFile/Test.h"
    # Check if file exists.
    if not os.path.exists(cpp_file_path):
        print(f"Error: the C++ file not found: {cpp_file_path}")
        sys.exit(1)

    if not os.path.exists(header_file_path):
        print(f"Error: the header file not found: {header_file_path}")
        sys.exit(1)

    # Run Cppcheck
    report = check_cpp_file(cpp_file_path)
